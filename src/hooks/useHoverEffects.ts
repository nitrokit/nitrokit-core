...
/**
 * Defines the available types of hover animation effects.
 */
export type HoverEffectType = 'none' | 'fireworks' | 'sparkles' | 'confetti' | 'ripple' | 'glow';

/**
 * Configuration options for the `useHoverEffects` hook.
 */
interface HoverEffectsOptions {
    /** The type of hover effect to trigger. */
    effect: HoverEffectType;
    /** Whether the hover effect is currently enabled. */
    enabled: boolean;
    /** The intensity of the effect, controlling particle count or size. Defaults to 'medium'. */
    intensity?: 'low' | 'medium' | 'high';
    /** The primary color for the effect particles (e.g., fireworks, ripple). */
    color?: string;
}

/**
 * A React hook that applies dynamic visual effects to an element on mouse hover.
 * It creates and animates DOM elements to produce effects like fireworks, sparkles, and ripples.
 *
 * @param {HoverEffectsOptions} options - Configuration for the hover effect.
 * @returns An object containing:
 *  - `containerRef`: A React ref to be attached to the element that will trigger the hover effects.
 *  - `handleMouseEnter`: A callback function to be passed to the `onMouseEnter` prop of the target element.
 */
