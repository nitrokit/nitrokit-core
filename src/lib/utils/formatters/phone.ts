...
export const formatPhoneForDisplay = (phone: string): string => {
    ...
    const withoutPlus = phone.replace(/[^\d]/g, '');
    ...
    if (!countryCode) return `+${withoutPlus}`;
...
export const getCountryFlag = (phone: string): string => {
    const digits = phone.replace(/[^\d]/g, ''); // Get only the digits
    let longestMatch = '';

    // Find the longest key from the map that is a prefix of the phone number
    for (const code in COUNTRY_FLAGS_MAP) {
        if (digits.startsWith(code) && code.length > longestMatch.length) {
            longestMatch = code;
        }
    }

    return longestMatch ? COUNTRY_FLAGS_MAP[longestMatch] : DEFAULT_FLAG;
};
